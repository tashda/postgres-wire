...
public class PostgresDockerManager: @unchecked Sendable {
    ...
    private let logger = Logger(label: "postgres.wire.tests")
    ...
    public func startIfNeeded() throws {
        ...
        logger.info("Starting Postgres \(version) on port \(port)")
        ...
        logger.info("Container started: \(output)")
        ...
        for path in possiblePaths {
            if fm.fileExists(atPath: path) {
                logger.info("Found sample data at: \(path)")
        ...
        if !loaded {
            logger.warning("SampleData.sql not found in any of the expected locations")
        }
    ...
    private func waitForReady(dockerPath: String) throws {
        ...
        for i in 1...maxRetries {
            ...
            if process.terminationStatus == 0 {
                logger.info("Postgres is ready after \(i) seconds")
                return
            }
    ...
    public func runSQLFile(at path: String, dockerPath: String) throws {
        logger.info("Loading SQL file content")
