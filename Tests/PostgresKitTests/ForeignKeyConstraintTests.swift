...
final class ForeignKeyConstraintTests: PostgresKitTestCase {
    ...
    override func setUp() async throws {
        ...
        client = try await PostgresDatabaseClient.connect(configuration: config, logger: Logger(label: "fk-constraint-tests"))
    }
    ...
    func testForeignKeyEnforcesReferentialIntegrity() async throws {
        ...
        _ = try await client.insert(into: parent, columns: ["name"], values: [["Parent 1"]])
        let rows = try await client.simpleQuery("SELECT id FROM \(parent) LIMIT 1")
        var parentId: Int?
        for try await id in rows.decode(Int.self) { parentId = id }

        // Valid FK reference
        _ = try await client.insert(into: child, columns: ["parent_id", "value"], values: [[parentId!, "Child 1"]])

        // Invalid FK reference
        do {
            _ = try await client.insert(into: child, columns: ["parent_id", "value"], values: [[99999, "Orphan"]])
    ...
    func testForeignKeyCascadeDelete() async throws {
        ...
        _ = try await client.insert(into: parent, columns: ["name"], values: [["To Delete"]])
        let rows = try await client.simpleQuery("SELECT id FROM \(parent) LIMIT 1")
        var parentId: Int?
        for try await id in rows.decode(Int.self) { parentId = id }

        _ = try await client.insert(into: child, columns: ["parent_id", "value"], values: [
            [parentId!, "Child A"],
            [parentId!, "Child B"]
        ])
    ...
    func testForeignKeyRestrictDelete() async throws {
        ...
        _ = try await client.insert(into: parent, columns: ["name"], values: [["Protected"]])
        let rows = try await client.simpleQuery("SELECT id FROM \(parent) LIMIT 1")
        var parentId: Int?
        for try await id in rows.decode(Int.self) { parentId = id }
    ...
    func testForeignKeyWithUUID() async throws {
        ...
        _ = try await client.addPrimaryKey(table: parent, column: "id", constraintName: "\(parent)_pk")
        ...
        let parentUUID = UUID()
        _ = try await client.insert(into: parent, columns: ["id", "name"], values: [[parentUUID, "UUID Parent"]])
        _ = try await client.insert(into: child, columns: ["parent_uuid", "value"], values: [[parentUUID, "UUID Child"]])

        // Non-existent UUID should fail
        do {
            _ = try await client.insert(into: child, columns: ["parent_uuid", "value"], values: [[UUID(), "Orphan"]])
